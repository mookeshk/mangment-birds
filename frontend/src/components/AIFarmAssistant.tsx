...
export const AIFarmAssistant = () => {
    ...
    const [messages, setMessages] = useState([{ role: 'model', text: 'مرحباً! أنا مساعد مزرعتي الذكي. يمكنني مساعدتك في ضبط الإعدادات واكتشاف خصائص الطيور. كيف أخدمك اليوم؟' }]);
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            // Mock AI response since no API key is provided
            setTimeout(() => {
                setMessages(prev => [...prev, { role: 'model', text: 'هذه استجابة تجريبية من المساعد الذكي. يمكنك لاحقاً ربطي بـ Gemini API للإجابة على جميع استفساراتك!' }]);
                setIsLoading(false);
            }, 1000);
        } catch (error) {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50" dir="rtl">
                ...
                <div className="bg-white dark:bg-[#1e293b] rounded-3xl shadow-2xl border border-gray-100 dark:border-[#334155] w-[22rem] sm:w-[24rem] mb-4 flex flex-col h-[32rem] transform origin-bottom-right transition-all animate-in fade-in zoom-in">
                    <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 p-4 text-white flex justify-between items-center shadow-md rounded-t-3xl">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-sm">مساعد الإعدادات</h3>
                        ...
                        <button onClick={() => setIsOpen(false)} className="text-white/80 hover:text-white hover:bg-white/10 p-1 rounded-lg transition-colors focus:outline-none">
                            <Icons.Close />
                        </button>
                    </div>
                    ...
                    <form onSubmit={handleSend} className="p-3 bg-white dark:bg-[#1e293b] border-t border-gray-100 dark:border-[#334155] flex gap-2 items-center rounded-b-3xl">
                        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="اسأل عن إعدادات المزرعة..." className="flex-1 px-4 py-2.5 bg-gray-100 dark:bg-[#0f172a] border border-transparent focus:border-emerald-500/30 rounded-full text-sm focus:outline-none focus:ring-0 text-gray-800 dark:text-white transition-all"/>
                        <button type="submit" disabled={isLoading || !input.trim()} className="bg-emerald-500 text-white p-2.5 rounded-full hover:bg-emerald-600 disabled:opacity-50 transition-colors flex-shrink-0 focus:outline-none">
            ...
            {!isOpen && (
                <button onClick={() => setIsOpen(true)} className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl w-14 h-14 shadow-lg shadow-emerald-500/30 flex items-center justify-center transition-transform duration-300 hover:scale-105 focus:outline-none">
                    <Icons.Sparkles />
                </button>
            )}
        </div>
